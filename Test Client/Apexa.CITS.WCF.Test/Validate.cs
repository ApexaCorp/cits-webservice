...
namespace Apexa.CITS.WCF.Test
{
    ...
    {
        internal static List<string> GetFeedErrors(TXLife_Type response)
        {
            var errors = new List<string>();

            var olife = response.Items.OfType<TXLifeResponse_Type>().Select(x => x.OLifE).FirstOrDefault();
            var advisors = olife.Items?.OfType<Party_Type>().Where(p => p.PartyTypeCode.Value == "Person") ?? new List<Party_Type>();

            foreach (var advisor in advisors ?? new List<Party_Type>())
            {
            ...
            foreach (var org in organizations ?? new List<Party_Type>())
            {
            ...
            foreach (var partner in partners ?? new List<Party_Type>())
            {
            ...
            foreach (var shareholder in shareholders ?? new List<Party_Type>())
            {
        ...
        internal static IEnumerable<string> Appointments(TXLife_Type response, Party_Type contractor)
        {
            ...
            foreach (var app in contractor.Producer.CarrierAppointment ?? new CarrierAppointment_Type[0])
            {
                ...
                foreach (var govId in app.GovtIDInfo ?? new GovtIDInfo_Type[0])
                {
                    if (LUObjNotValid(govId)) { errors.Add("Appointment GovtIDInfo"); }
                }
                foreach (var assocInfo in app.AssocCarrierApptInfo ?? new AssocCarrierApptInfo_Type[0])
                {
                ...
                // Referral
                foreach (var referral in app.ReferralInfo ?? new ReferralInfo_Type[0])
                {
                ...
                // Payment
                foreach (var distrib in app.DistributionAgreementInfo ?? new DistributionAgreementInfo_Type[0])
                {
                    if (distrib.BankingInfoID != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                foreach (var attachment in holding.Attachment ?? new Attachment_Type[0])
                                {
                ...
                // Hierarchy
                foreach (var distribLevel in app.DistributionLevel ?? new DistributionLevel_Type[0])
                {
                ...
                // Consolidations
                foreach (var consolidation in app.ConsolidationInfo ?? new ConsolidationInfo_Type[0])
                {
                ...
                // Transfers
                foreach (var transfer in app.TransferInfo ?? new TransferInfo_Type[0])
                {
                    if (string.IsNullOrWhiteSpace(transfer.CarrierAppointmentID)) { errors.Add("Appointment transfer CarrierAppointmentID"); }
                }

                // Debts
                foreach (var debt in app.DebtInfo ?? new DebtInfo_Type[0])
                {
                ...
                foreach (var req in app.RequirementInfo ?? new RequirementInfo_Type[0])
                {
                    ...
                    foreach (var attachment in req.Attachment ?? new Attachment_Type[0])
                    {
                        errors.AddRange(Attachment(attachment, "Appointment Requirements"));

                        foreach (var signature in attachment.SignatureInfo ?? new SignatureInfo_Type[0])
                        {
                ...
                {
                    errors.Add("Appointent CustomQuestions formInstance");
                }
                else
                {
                    ...
                    foreach (var formResponse in formInstance.FormResponse ?? new FormResponse_Type[0])
                    {
        ...
        internal static IEnumerable<string> OrganizationIdentity(Party_Type party)
        {
            ...
            foreach (var id in party.GovtIDInfo ?? new GovtIDInfo_Type[0])
            {
                if (LUObjNotValid(id)) { errors.Add("organization GovtIDInfo"); }
            }
            foreach (var attachment in party.Attachment ?? new Attachment_Type[0])
            {
        ...
        internal static IEnumerable<string> Addresses(Address_Type[] addresses, string module)
        {
            ...
            if (addresses == null) return errors;
        ...
        internal static IEnumerable<string> CLHIA(TXLife_Type response, Party_Type contractor)
        {
            ...
            if (!long.TryParse(Regex.Match(contractor.id, "C([0-9]+)").Groups[1].Value, out long contractorEntityId)) { errors.Add("CLHIA contractorEntityId"); }
            else
            {
                if (contractor.Producer != null)
                {
                    ...
                    {
                        foreach (var approval in contractor.Producer.NationApproval ?? new NationApproval_Type[0])
                        {
        ...
        {
            var errors = new List<string>();
            foreach (var debt in debts ?? new Holding_Type[0])
            {
        ...
        internal static IEnumerable<string> Debt(TXLife_Type response, Holding_Type debt, string module)
        {
            ...
            foreach (var loan in debt.Loan ?? new Loan_Type[0])
            {
            ...
            if (!long.TryParse(Regex.Match(debt.id, "DEBT([0-9]+)").Groups[1].Value, out long debtId)) { errors.Add(module + " debt id"); }
        ...
        internal static IEnumerable<string> EOCoverage(TXLife_Type response, Party_Type contractor)
        {
            ...
            foreach (var coverage in coverages ?? new List<Holding_Type>())
            {
        ...
        internal static IEnumerable<string> Supervision(Party_Type contractor)
        {
            ...
            if (contractor.Producer?.SupervisionLevel == null) return errors;
